...
public class EitherInstanceMethodsTest {
    ...
    public void bimap_left() {
        Either<String, Object> eitherLeft = Either.left("not found").bimap(left -> left + " mapped", Function.identity()
        );
    ...
    public void bimap_right() {
        Either<Object, String> eitherLeft = Either.right("not found").bimap(Function.identity(), left -> left + " mapped"
        );
    ...
    public void filter_left() {
        Option<Either<Integer, Integer>> filter = Either.<Integer, Integer>left(6)
                .filter(x -> x > 10);
    ...
    public void filter_right_notSatisfied() {
        Option<Either<Integer, Integer>> filter = Either.<Integer, Integer>right(6)
                .filter(x -> x > 10);
